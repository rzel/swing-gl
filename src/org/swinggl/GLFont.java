...
/**
 * <p>creates GL renderable (blittable) font out of given AWT font.</p>
 *  
 * <p>this class uses {@link TexturePack} behind the scenes.<p>
 * 
 * @see TexturePack 
 * 
 * @author hakan eryargi (r a f t)
 */
// TODO: we need to separate character width and character image width.
// former is used to advance cursor, later is to blit character.
// making both same creates problem for italic fonts for example
public class GLFont {
	/** standard characters */
	...
	final TexturePack pack = new TexturePack();
	 ...
	 * @param y baseline
	 */
	public void blitString(String s, int x, int y, Color color) {
		if (s.length() == 0)
			return;
		...
		try {
			for (int i = 0; i < s.length(); i++) {
				...
				if (index == -1)
					index = alphabet.indexOf('?');
				if (index != -1) {
					Dimension size = pack.blitOnly(index, x, y);
					x += size.width;
				}
			}
		} finally {
